...
from unittest.mock import patch, Mock, call as mock_call, _Call
...
def test_with_fail_download(ytdlp_mock: Mock):
  download_mock: Mock = Mock(side_effect=Exception())
  class fakeYtdlp:
  ...
  with pytest_raises(Exception):
    DownloadSection()._download_item('invalid url', {}, retry=2) # type: ignore
...
def test_download_video (
  ...
  @dataclass
  class Case:
    dl_opt: Union[dict[str, Any], BundledTDownloadOpt]
    sub_langs: list[str]
  ...
  fake_bundled_v = { 'v': 'v' }
  fake_bundled_a = { 'a': 'a' }
  fake_bundled_dl_opt = BundledTDownloadOpt(video=fake_bundled_v, audio=fake_bundled_a) # type: ignore
  # fake subtitle
  fake_sub_lang = 'en'
  ...
  case_ls : list[Case] = [
    ...
    Case(fake_dl_opt, []),
    # bundled, no subtitle
    Case(fake_bundled_dl_opt, []),
    # not bundled, has subtitle
    Case(fake_dl_opt, [fake_sub_lang]),
  ]
  
  for case in case_ls:
    print('testing', case)
    case_sub_opt = { 'subtitleslangs': case.sub_langs }
    ...
    to_dl_opt_mock.return_value = case.dl_opt
    ...
    is_bundle = isinstance(case.dl_opt, BundledTDownloadOpt)
    has_sub = len(case.sub_langs) > 0
    
    # expected download call
    expected_download_calls: list[_Call] = []
    if is_bundle:
      ...
      expected_download_calls.append(mock_call(fake_url, case.dl_opt, 0))
      
    if has_sub:
      expected_download_calls.append(mock_call(fake_url, case_sub_opt, 0))

    DownloadSection()._download_video(opts, retry=0) # type: ignore
...
def test_downloadOpt_to_sub_opt():
  ...
  for case in case_ls:
    print('testing', case.sub)
    opts = DownloadOpt()
    opts.url = fake_url
    opts.output_dir = fake_output_dir
    opts.subtitle = case.sub
    opts.cookie_file_path = 'cookie_file_path'
    opts.login_browser = 'login_browser'
    ...
    if case.sub is None:
      ...
      assert sub_opt['writesubtitles'] == (not case.sub.isAuto)
      assert sub_opt['writeautomaticsub'] == case.sub.isAuto
      assert sub_opt['subtitleslangs'] == [case.sub.code]
      assert sub_opt['skip_download'] == True
# ========== Download option ========== #
